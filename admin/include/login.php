...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
  ...
  if ($tipo_usuario=='Empleado') {
    ...
    if ($nueva_consulta=$mysqli->prepare("SELECT A.Id,A.Tipo_Usuario,B.Primer_Nombre,B.Primer_Apellido FROM erp_usuario A INNER JOIN erp_cliente B ON (A.Id_Usuario=B.Id AND A.Usuario=? AND A.Id_Estado=1 AND A.Clave=sha2(?,'sha512'))")) {
      $nueva_consulta->bind_param('ss',$usuario,$clave);
      $nueva_consulta->execute();
      $rs=$nueva_consulta->get_result();
